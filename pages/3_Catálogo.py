...
if not rec.empty:
    ...
    # Agregar Laboratorio si existe (para enriquecer datos si mercado no lo tiene)
...
# Agregar nombres de laboratorios si están disponibles
if not lab_lookup.empty and "Laboratorio" in df.columns:
    df = df.merge(lab_lookup, on="Laboratorio", how="left")

# Si Laboratorio no existe pero rec tiene datos, intentar agregarla
if "Laboratorio" not in df.columns and not rec.empty and "Laboratorio" in rec.columns:
    rec_lab = rec[["IdArticulo", "Laboratorio"]].copy()
    rec_lab["IdArticulo"] = rec_lab["IdArticulo"].astype(str).str.strip()
    rec_lab = rec_lab.rename(columns={"IdArticulo": "CN"})
    if "CN" in df.columns:
        df = df.merge(rec_lab, on="CN", how="left")
...
# Filtro por Laboratorio (Marca) - prioriza Laboratorio si existe
lab_col = None
if "Laboratorio" in df.columns:
    lab_col = "Laboratorio"
elif "LaboratorioNombre" in df.columns:
    lab_col = "LaboratorioNombre"

if lab_col:
    lab_options = sorted([l for l in df[lab_col].dropna().unique() if l != ""])
    if lab_options:
